...
export async function GET(request: Request) {
  try {
    ...
    // Check if liquid amount is sufficient (at least 1 NEAR = 10^24 yoctoNEAR)
    const minAmount = "1000000000000000000000000"; // 1 NEAR in yoctoNEAR
    if (BigInt(liquidAmount) < BigInt(minAmount)) {
      return NextResponse.json({ 
    ...
    console.error('Error generating lock NEAR transaction payload:', error);
    return NextResponse.json({ error: 'Failed to generate lock NEAR transaction payload' }, { status: 500 });
  }
} 