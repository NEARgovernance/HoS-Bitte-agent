import { NextResponse } from 'next/server';
import { VENEAR_CONTRACT_ID, NEAR_RPC_URL } from '@/app/config';

// Type definitions for veNEAR account info
interface VeNEARDelegation {
  account_id: string;
  amount: string;
}

interface VeNEARAccount {
  account_id: string;
  balance: string;
  delegation: VeNEARDelegation | null;
}

interface VeNEARAccountInfo {
  account: VeNEARAccount;
}

// Convert Tgas to gas units (1 Tgas = 10^12 gas)
function tgasToGas(tgas: string): string {
  const tgasValue = parseFloat(tgas);
  return (tgasValue * 1e12).toString();
}

// Get account info from veNEAR contract
async function getAccountInfo(accountId: string): Promise<{ accountInfo: VeNEARAccountInfo } | NextResponse> {
  if (!VENEAR_CONTRACT_ID) {
    return NextResponse.json({ error: 'VENEAR_CONTRACT_ID environment variable not set' }, { status: 500 });
  }

  const payload = {
    jsonrpc: "2.0",
    id: "1",
    method: "query",
    params: {
      request_type: "call_function",
      finality: "final",
      account_id: VENEAR_CONTRACT_ID,
      method_name: "get_account_info",
      args_base64: Buffer.from(JSON.stringify({ account_id: accountId })).toString("base64"),
    },
  };

  try {
    const res = await fetch(NEAR_RPC_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    
    if (!res.ok) {
      return NextResponse.json({ error: `RPC request failed: ${res.status}` }, { status: 500 });
    }

    const json = await res.json();

    if (json.error) {
      return NextResponse.json({ error: `RPC error: ${json.error.message}` }, { status: 500 });
    }

    if (!json.result || !json.result.result || json.result.result.length === 0) {
      return NextResponse.json({ error: 'Failed to get account info' }, { status: 500 });
    }

    // Convert byte array to string, then parse JSON
    const bytes = json.result.result;
    const raw = Buffer.from(bytes).toString("utf-8");
    const accountInfo = JSON.parse(raw);

    return { accountInfo };
  } catch (error) {
    console.error('Error getting account info:', error);
    return NextResponse.json({ error: 'Failed to get account info' }, { status: 500 });
  }
}

export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const accountId = searchParams.get('accountId');

    if (!accountId) {
      return NextResponse.json({ 
        error: 'accountId parameter is required' 
      }, { status: 200 });
    }

    if (!VENEAR_CONTRACT_ID) {
      return NextResponse.json({ 
        error: 'VENEAR_CONTRACT_ID environment variable not set' 
      }, { status: 500 });
    }

    // Get account info to check if user is delegating
    const accountInfoResult = await getAccountInfo(accountId);
    if (accountInfoResult instanceof NextResponse) {
      return accountInfoResult;
    }
    const { accountInfo } = accountInfoResult;

    // Check if user is currently delegating
    const isDelegating = !!accountInfo?.account?.delegation;
    if (!isDelegating) {
      return NextResponse.json({ 
        error: 'Account is not currently delegating any veNEAR tokens',
        accountInfo
      }, { status: 200 });
    }

    // Create the undelegate all veNEAR transaction payload
    const transactionPayload = {
      receiverId: VENEAR_CONTRACT_ID,
      actions: [
        {
          type: "FunctionCall",
          params: {
            methodName: "undelegate",
            gas: tgasToGas("100"), // 100 Tgas
            deposit: "1", // 1 yoctoNEAR
            args: {}
          }
        }
      ]
    };

    return NextResponse.json({ 
      transactionPayload,
      delegationInfo: accountInfo.account.delegation
    });

  } catch (error) {
    console.error('Error generating undelegate all veNEAR transaction payload:', error);
    return NextResponse.json({ error: 'Failed to generate undelegate all veNEAR transaction payload' }, { status: 500 });
  }
} 